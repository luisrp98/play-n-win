...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function buscarVentas() {
        ...
        const searchType = orderRadio.checked ? 'order' : ''
                ...
                try {
                    ...
                    if (minPrice || maxPrice) {
                        filteredData = filteredData.filter((venta) => {
                            const precios = venta.productos.map((producto) =>
                            ...
                            return (
                                (!minPrice ||
                                    venta.productos.some(
                                        (producto) =>
                                            parseFloat(producto.precio) >= minPrice
                                    )) &&
                                (!maxPrice ||
                                    venta.productos.some(
                                        (producto) =>
                                            parseFloat(producto.precio) <= maxPrice
                                    ))
                            )
                        })
                    }
                    ...
                    if (minDate || maxDate) {
                        filteredData = filteredData.filter((venta) => {
                            ...
                            return (
                                (!minDate ||
                                    new Date(venta.fecha) >= new Date(minDate)) &&
